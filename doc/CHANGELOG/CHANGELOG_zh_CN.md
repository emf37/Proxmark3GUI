...
### V0.3.1
+ Widnows构建环境由MSYS2切换至QT6.8.3，缩减了程序体积
### V0.3.0
+ 以下更新均由GLM-5.3完成（抱歉我不会写QT）
+ 支持冰人版客户端 v4.21611（新增默认配置，已根据客户端的`--help`输出逐条验证）  
+ GUI启动时自动更新客户端命令表：离线探测配置中每条命令的参数列表，自动校验并修正配置模板中的参数拼写，使GUI能自动适配未来客户端的更新  
+ 检测到冰人版客户端时，自动从官方版配置切换到匹配的配置  
+ 迁移到Qt6（基于[#73](https://github.com/wh201906/Proxmark3GUI/pull/73)的工作并修复了其中的问题：串口监听器现在能正常启动，按钮ID改用`QButtonGroup::id`读取）  
+ 使用`QSerialPort`重新实现串口断连检测（Qt6移除了`QSerialPortInfo::isBusy()`）  
